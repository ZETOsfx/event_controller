<template>
    ...
    <div class="intro">
        <div class="container">
            ...
            <!-- Error Modal -->
            <div v-if="errorFlag !== -1" class="modal fade show" id="staticErrorModal" data-bs-backdrop="static" tabindex="-1" style="color: black; display: block;">
              <div class="modal-dialog">
                <!-- Переполнение в имени события при добавлении (огрн: 50 сим) -->
                <div v-if="errorFlag === 0" class="modal-content">
                  <div class="modal-header">
                    <h1 class="modal-title fs-5" id="staticBackdropLabel">Ошибка добавления</h1>
                ...
                <div v-if="errorFlag === 2" class="modal-content">
                  <div class="modal-header">
                    <h1 class="modal-title fs-5" id="staticBackdropLabel">Ошибка создания шаблона</h1>
                    <button @click="triggerStupid()" type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                  </div>
                  <div class="modal-body">
                ...
                <div v-if="errorFlag === 3" class="modal-content">
                  <div class="modal-header">
                    <h1 class="modal-title fs-5" id="staticBackdropLabel">Ошибка открытия шаблона</h1>
                    <button @click="triggerStupid()" type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                  </div>
                  <div class="modal-body">
                ...
                <div v-if="errorFlag === 6" class="modal-content">
                  <div class="modal-header">
                    <h1 class="modal-title fs-5" id="staticBackdropLabel">Ошибка удаления шаблона</h1>
                    <button @click="triggerStupid()" type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                  </div>
                  <div class="modal-body">
            ...
            <div v-if="errorFlag !== -1" class="modal-backdrop fade show"> </div>
            ...
            <div class="content">
                <div style="margin-right: 0px"> 
                  ...
                  <!-- Modal for Save -->
                  <div class="modal fade" id="ModalSave" tabindex="-1" aria-hidden="true" style="color:black">
                      <div class="modal-dialog">
                  ...
                  <!-- Modal for Delete -->
                  <div class="modal fade" id="ModalDelete" tabindex="-1" aria-hidden="true" style="color:black">
                      <div class="modal-dialog">
                          <div class="modal-content">
                              <div class="modal-header">
                ...
                <div v-if="isGet" class="d-flex w-100 justify-content-start align-items-center gap-3 mt-1">
                    ...
                    <div v-if="isAuthor" class="form-check form-switch mt-1">
                        <input @click="doesSomething()"
                              class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault" v-model="canView" v-bind:checked="canView">
                    ...
                    <div v-if="canView && isAuthor" class="form-check form-switch mt-1">
                        <input @click="doesSomething()"
                              class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault" v-model="canEdit" v-bind:checked="canEdit">
                        <label class="form-check-label" for="flexSwitchCheckDefault">Разрешить редактировать другим</label>
                    </div>
                </div>
            </div>
              <!-- Modal for Send -->
              <div class="modal fade" id="SendModerModal" tabindex="-1" aria-hidden="true" style="color:black">
                  <div class="modal-dialog">
                      <div class="modal-content">
                          <div class="modal-header">
                          ...
                          <div class="modal-footer">
                              <div class="d-flex w-100 justify-content-between align-items-center gap-1">
                                  ...
                                  <select class="form-select form-select-sm w-75" aria-label=".form-select-sm example">
                                      <option selected>- Нет изменений -</option>
                              ...
                              <div class="d-flex w-100 justify-content-between gap-1">
                                  <select class="form-select" aria-label="Default select example">
                                      <option selected>Экран отображения</option>
                                      <option value="1">Кафедра К3 - основной</option>
                                      <option value="2">Кафедра К3 - новости</option>
                                      <option value="3">Хол актового</option>
                                  </select>
                              ...
                              <div class="d-flex w-100 justify-content-between align-items-center">
                                  ...
                                  <div>       
                                      ...
                                      <button type="button" class="btn btn-success ms-2"> Отправить </button>
                                  </div>
                              </div>
                          </div>
                      </div>
          ...
          <div v-if="isGet" class="content">
              ...
              <div v-for="(event, index) in events" class="row gx-3 gy-2 align-items-center p-1">
                <div v-if="!editForm[index].isDeleted" class="col-sm-3">
                  ...
                  <input v-model="event.name" type="text" class="form-control" id="specificSizeInputName" placeholder="Имя"
                          :disabled="editForm[index].isDisabled">
                </div>
                <div v-if="!editForm[index].isDeleted" class="col-sm-2">
                  <label class="visually-hidden" for="specificSizeInputGroupUsername">Username</label>
                  <input v-model="event.src" type="text" class="form-control" id="specificSizeInputGroupUsername"
                ...
                <div v-if="!editForm[index].isDeleted" class="col-sm-1">
                  <label class="visually-hidden" for="specificSizeInputGroupUsername">Username</label>
                  <input v-model="event.time" type="number" class="form-control" id="specificSizeInputGroupUsername" placeholder="Время в сек" :disabled="editForm[index].isDisabled">
                </div>
                ...
                <div v-if="!editForm[index].isDeleted" class="col-auto">
                  <button v-if="!editForm[index].isDeleted && editForm[index].isDisabled && (canEdit || isAuthor)" @click="moveEvent('up', index)" class="btn btn-outline-warning"><i class="fa-solid fa-up"></i></button>
                </div>
                <div v-if="!editForm[index].isDeleted" class="col-auto">
                  <button v-if="!editForm[index].isDeleted && editForm[index].isDisabled && (canEdit || isAuthor)" @click="moveEvent('down', index)" class="btn btn-outline-warning"><i class="fa-solid fa-down"></i></button>
                </div>
              </div>
              ...
              <div v-if="canEdit || isAuthor" class="row gx-3 gy-2 align-items-center p-1">
                ...
                <div class="col-sm-2">
                  <label class="visually-hidden" for="specificSizeInputGroupUsername">Username</label>
                  <input v-model="addForm.src" type="text" class="form-control" id="specificSizeInputGroupUsername"
                ...
                <div class="col-sm-1">
                  <label class="visually-hidden" for="specificSizeInputGroupUsername">Username</label>
                  <input v-model="addForm.time" type="number" class="form-control" id="specificSizeInputGroupUsername"
                          placeholder="Время в сек" required>
                </div>
